// RUN: %clang_cc1 -plugin analyzer -plugin-arg-analyzer -analyzer-checker=experimental.unix.Stream -plugin-arg-analyzer -analyzer-store -plugin-arg-analyzer region -verify %s
