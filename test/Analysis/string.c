...
// RUN: %clang_cc1 -plugin analyzer -DVARIANT -plugin-arg-analyzer -analyzer-checker=core,experimental.unix.CString,experimental.deadcode.UnreachableCode -plugin-arg-analyzer -analyzer-store=region -Wno-null-dereference -verify %s
// RUN: %clang_cc1 -plugin analyzer -DUSE_BUILTINS -DVARIANT -plugin-arg-analyzer -analyzer-checker=experimental.security.taint,core,experimental.unix.CString,experimental.deadcode.UnreachableCode -plugin-arg-analyzer -analyzer-store=region -Wno-null-dereference -verify %s
